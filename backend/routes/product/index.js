const router = require('express').Router();
const mysql = require('mysql2/promise');
const pool = mysql.createPool({
  host: 'localhost',
  user: 'root',
  password: 'system',
  database: 'Kodnestbank',
  waitForConnections: true,
  connectionLimit: 10,
  queueLimit: 0,
});


router.get("/product", async (req, res) => {
    try {
        const { q } = req.query; // Get the search query from the request
        let rows;

        if (q) {
            // If a search query is provided, filter the results
            [rows] = await pool.query(
                "SELECT * FROM Product WHERE Product_name LIKE ?", 
                [`%${q}%`] // Use wildcards for partial matching
            );
        } else {
            // If no search query, return all rows
            [rows] = await pool.query("SELECT * FROM Product");
        }

        res.send({ data: rows });
    } catch (error) {
        console.error(error);
        res.status(500).send({ error: true, message: "Internal Server Error" });
    }
}).post("/product/new",async (req,res)=>{

    const {pname,Sub_Category,category} = req.body;

    //console.log("Image",image)
   

    await pool.query("INSERT INTO Product (Product_name,Sub_Category,category,Status) VALUES (?,?,?,?)", [pname, Sub_Category,category,"INACTIVE"]);

    res.send({ error: false, message: "Product Created" });
}).patch("/product/:id", async (req, res) => {
    const { id } = req.params; // Get the Product ID from the URL params
    const payload = req.body; // Get the update payload from the request body

    try {
        // Check if the payload is empty
        if (Object.keys(payload).length === 0) {
            return res.status(400).send({ error: true, message: "No fields to update" });
        }

        // Dynamically build the SET clause for the UPDATE query
        const fields = Object.keys(payload)
            .map((key) => `${key} = ?`) // e.g., "name = ?", "description = ?"
            .join(", ");

        const values = Object.values(payload); // Get the values to bind

        // Add the ID to the values for the WHERE clause
        values.push(id);

        // Execute the query
        const [result] = await pool.query(
            `UPDATE Product SET ${fields} WHERE id = ?`,
            values
        );

        // Check if any row was affected
        if (result.affectedRows === 0) {
            return res.status(404).send({ error: true, message: "Product not found" });
        }

        res.send({ error: false, message: "Product updated successfully" });
    } catch (error) {
        console.error(error);
        res.status(500).send({ error: true, message: "Internal Server Error" });
    }
}).delete("/product/:id", async (req, res) => {
    const { id } = req.params; // Extract the Product ID from the request parameters

    try {
        // Execute the DELETE query
        const [result] = await pool.query("DELETE FROM Product WHERE id = ?", [id]);

        // Check if any row was affected
        if (result.affectedRows === 0) {
            return res.status(404).send({ error: true, message: "Product not found" });
        }

        res.send({ error: false, message: "Product deleted successfully" });
    } catch (error) {
        console.error(error);
        res.status(500).send({ error: true, message: "Internal Server Error" });
    }
});

module.exports = router;